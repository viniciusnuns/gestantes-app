-- Fix admin_users_list to show current calculated pregnancy week
-- instead of the static 'week' column stored at registration.
...
SELECT
  ...
  -- Calculated current week (progresses with time since registration)
  LEAST(
  ...
  -- Trimester based on calculated current week
  CASE
    WHEN LEAST(
      COALESCE(u.week_at_registration, u.week, 20) +
      FLOOR(EXTRACT(EPOCH FROM (NOW() - COALESCE(u.registration_date, u.created_at, NOW()))) / 604800)::INTEGER,
