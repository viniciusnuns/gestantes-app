...
const inputCls = 'w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-300 focus:border-transparent bg-white'
...
export default function EbookGestacaoCheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/checkout/ebook-gestacao', {
        method: 'POST',
      ...
      if (data.confirmed && data.billingType === 'CREDIT_CARD') {
        router.push('/ebook-gestacao/sucesso?metodo=cartao')
        return
      }
      ...
      if (data.billingType === 'PIX') {
        sessionStorage.setItem('pix_data', JSON.stringify({
          ...
          email: user.email,
          value: data.value,
          successUrl: '/ebook-gestacao/sucesso',
        }))
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        {/* Hero */}
        <div className="flex items-center gap-4 bg-gradient-to-br from-amber-50 to-yellow-100 rounded-2xl p-5 border border-amber-200">
          ...
          <div className="flex-1">
            <p className="text-xs font-bold text-amber-600 uppercase tracking-wide mb-0.5">Ebook Digital</p>
            <h1 className="text-lg font-black text-amber-900 leading-tight">Gestante Bem Informada</h1>
            <p className="text-sm text-amber-700">Gestação — Dra. Fabiana Pinheiro</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          <form onSubmit={handleSubmit}>
            ...
            <div className="p-5 space-y-4">
              ...
              {/* Preço */}
              <div className={`rounded-xl px-4 py-3 flex items-center justify-between ${billingType === 'PIX' ? 'bg-emerald-50 border border-emerald-200' : 'bg-amber-50 border border-amber-200'}`}>
                <div>
                  <p className={`text-sm font-bold ${billingType === 'PIX' ? 'text-emerald-800' : 'text-amber-800'}`}>
                    {billingType === 'PIX' ? 'PIX · à vista' : 'Cartão · à vista'}
                  </p>
                  <p className={`text-xs mt-0.5 ${billingType === 'PIX' ? 'text-emerald-600' : 'text-amber-600'}`}>
                    Acesso imediato ao ebook em PDF
                  </p>
                </div>
                <p className={`text-2xl font-black ${billingType === 'PIX' ? 'text-emerald-800' : 'text-amber-800'}`}>
                  R$17
                </p>
              </div>
              ...
              {billingType === 'CREDIT_CARD' && (
                <>
                  <CardFields
                    ...
                    installments={[{ count: 1, value: EBOOK_GESTACAO_PRICE, display: `1x R$${EBOOK_GESTACAO_PRICE.toFixed(2).replace('.', ',')}`, total: EBOOK_GESTACAO_PRICE }]}
                  />
                  <input
                    type="tel"
                    value={phone}
