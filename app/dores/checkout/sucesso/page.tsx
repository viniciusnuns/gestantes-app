...
import { DORES_PIX_PRICE } from '@/lib/checkout-config'
...
function SucessoContent() {
  ...
  useEffect(() => {
    ...
    const pixelValue = valueParam ? parseFloat(valueParam) : DORES_PIX_PRICE
    if (paymentId) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12"
      style={{ background: 'linear-gradient(135deg, #F3F0FF 0%, #FDF4F8 100%)' }}>
      ...
      <div className="w-full max-w-md text-center space-y-8">
        ...
        <div className="space-y-3">
          ...
          <p className="text-gray-600 text-lg">
            Suas sequências para alívio de dores estão liberadas.
          </p>
          {metodo === 'cartao' && (
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 text-left space-y-3">
          ...
          {[
            ...
            'Exercícios seguros com a Dra. Fabiana Pinheiro',
        ...
        <div className="space-y-3">
          <button onClick={() => router.push('/onboarding')}
            className="w-full text-white font-black text-lg py-4 rounded-2xl shadow-md transition-all flex items-center justify-center gap-2"
...
export default function DoresSucessoPage() {
  return (
    <Suspense>
      <SucessoContent />
    </Suspense>
